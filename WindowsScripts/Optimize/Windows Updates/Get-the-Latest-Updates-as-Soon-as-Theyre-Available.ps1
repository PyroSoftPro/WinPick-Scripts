...
# DESCRIPTION: Disables early update opt-in. Use -Undo to enable it.
# UNDOABLE: Yes
# UNDO_DESC: Enables early update opt-in.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Early update opt-in enabled." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 0 -Type DWord -Force
        Write-Log "Early update opt-in disabled." "INFO"
    }
