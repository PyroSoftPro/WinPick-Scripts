...
# DESCRIPTION: Disables downloading updates over metered connections. Use -Undo to enable it.
# UNDOABLE: Yes
# UNDO_DESC: Enables downloading updates over metered connections.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Metered connection downloads enabled." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 0 -Type DWord -Force
        Write-Log "Metered connection downloads disabled." "INFO"
    }
