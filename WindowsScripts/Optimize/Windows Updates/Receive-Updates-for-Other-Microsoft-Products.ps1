...
# DESCRIPTION: Disables updates for other Microsoft products. Use -Undo to enable them.
# UNDOABLE: Yes
# UNDO_DESC: Enables updates for other Microsoft products.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Updates for other Microsoft products enabled." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 0 -Type DWord -Force
        Write-Log "Updates for other Microsoft products disabled." "INFO"
    }
