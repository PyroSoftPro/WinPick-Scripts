...
# DESCRIPTION: Disables expedited update restarts. Use -Undo to enable them.
# UNDOABLE: Yes
# UNDO_DESC: Enables expedited update restarts.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Expedited update restarts enabled." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 0 -Type DWord -Force
        Write-Log "Expedited update restarts disabled." "INFO"
    }
