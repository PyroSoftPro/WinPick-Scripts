...
# DESCRIPTION: Disables restart required notifications. Use -Undo to enable them.
# UNDOABLE: Yes
# UNDO_DESC: Enables restart required notifications.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Restart required notifications enabled." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 0 -Type DWord -Force
        Write-Log "Restart required notifications disabled." "INFO"
    }
