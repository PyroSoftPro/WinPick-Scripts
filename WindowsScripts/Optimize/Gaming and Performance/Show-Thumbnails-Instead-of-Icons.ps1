...
# DESCRIPTION: Shows thumbnails instead of icons. Use -Undo to show icons only.
# UNDOABLE: Yes
# UNDO_DESC: Shows icons only (no thumbnails).
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Icons only enabled (thumbnails disabled)." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 0 -Type DWord -Force
        Write-Log "Thumbnails enabled." "INFO"
    }
