...
# DESCRIPTION: Disables the Narrator hotkey. Use -Undo to enable it.
# UNDOABLE: Yes
# UNDO_DESC: Enables the Narrator hotkey.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Narrator hotkey enabled." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 0 -Type DWord -Force
        Write-Log "Narrator hotkey disabled." "INFO"
    }
