...
# DESCRIPTION: Disables the AutoChk Proxy task. Use -Undo to enable it.
# UNDOABLE: Yes
# UNDO_DESC: Enables the AutoChk Proxy task.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "AutoChk Proxy task enabled." "INFO"
    } else {
        Set-TaskState -Name $TaskName -Enable $false
        Write-Log "AutoChk Proxy task disabled." "INFO"
    }
