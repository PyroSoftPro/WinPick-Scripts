...
# DESCRIPTION: Sets Print Spooler to Manual. Use -Undo to set it to Automatic.
# UNDOABLE: Yes
# UNDO_DESC: Sets Print Spooler to Automatic.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Print Spooler set to Automatic." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 3 -Type DWord -Force
        Write-Log "Print Spooler set to Manual." "INFO"
    }
