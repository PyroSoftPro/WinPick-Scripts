...
# DESCRIPTION: Disables the Feedback DmClient task. Use -Undo to enable it.
# UNDOABLE: Yes
# UNDO_DESC: Enables the Feedback DmClient task.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Feedback DmClient task enabled." "INFO"
    } else {
        Set-TaskState -Name $TaskName -Enable $false
        Write-Log "Feedback DmClient task disabled." "INFO"
    }
