...
# DESCRIPTION: Disables Game Bar tips and hints. Use -Undo to enable them.
# UNDOABLE: Yes
# UNDO_DESC: Enables Game Bar tips and hints.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Game Bar tips and hints are now enabled." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 0 -Type DWord -Force
        Write-Log "Game Bar tips and hints are now disabled." "INFO"
    }
