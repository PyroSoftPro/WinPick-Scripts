...
# DESCRIPTION: Disables the port redirector service. Use -Undo to set it to Automatic.
# UNDOABLE: Yes
# UNDO_DESC: Sets the port redirector service to Automatic.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Remote Desktop port redirector service set to Automatic." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 4 -Type DWord -Force
        Write-Log "Remote Desktop port redirector service disabled." "INFO"
    }
