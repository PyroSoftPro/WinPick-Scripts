...
# DESCRIPTION: Disables minimize/maximize animations. Use -Undo to enable them.
# UNDOABLE: Yes
# UNDO_DESC: Enables minimize/maximize animations.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Minimize/maximize animations enabled." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value "0" -Type String -Force
        Write-Log "Minimize/maximize animations disabled." "INFO"
    }
