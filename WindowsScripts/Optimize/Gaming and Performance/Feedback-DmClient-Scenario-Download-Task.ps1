...
# DESCRIPTION: Disables the Feedback DmClient Scenario Download task. Use -Undo to enable it.
# UNDOABLE: Yes
# UNDO_DESC: Enables the Feedback DmClient Scenario Download task.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Feedback DmClient Scenario Download task enabled." "INFO"
    } else {
        Set-TaskState -Name $TaskName -Enable $false
        Write-Log "Feedback DmClient Scenario Download task disabled." "INFO"
    }
