...
# DESCRIPTION: Disables translucent selection rectangle. Use -Undo to enable it.
# UNDOABLE: Yes
# UNDO_DESC: Enables translucent selection rectangle.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Translucent selection rectangle enabled." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 0 -Type DWord -Force
        Write-Log "Translucent selection rectangle disabled." "INFO"
    }
