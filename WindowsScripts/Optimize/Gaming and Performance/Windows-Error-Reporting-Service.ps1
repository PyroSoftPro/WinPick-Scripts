...
# DESCRIPTION: Disables Windows Error Reporting. Use -Undo to set it to Automatic.
# UNDOABLE: Yes
# UNDO_DESC: Sets Windows Error Reporting to Automatic.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Windows Error Reporting set to Automatic." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 4 -Type DWord -Force
        Write-Log "Windows Error Reporting disabled." "INFO"
    }
