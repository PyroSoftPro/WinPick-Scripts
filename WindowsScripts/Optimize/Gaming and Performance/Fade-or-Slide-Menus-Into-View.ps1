...
# DESCRIPTION: Disables menu animations. Use -Undo to enable them.
# UNDOABLE: Yes
# UNDO_DESC: Enables menu animations.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Menu animations are now enabled." "INFO"
    } else {
        Set-UserPreferencesMaskBit -Enable $false
        Write-Log "Menu animations are now disabled." "INFO"
    }
