...
# DESCRIPTION: Disables the disk diagnostic data collector task. Use -Undo to enable it.
# UNDOABLE: Yes
# UNDO_DESC: Enables the disk diagnostic data collector task.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Disk Diagnostic Data Collector task enabled." "INFO"
    } else {
        Set-TaskState -Name $TaskName -Enable $false
        Write-Log "Disk Diagnostic Data Collector task disabled." "INFO"
    }
