...
# DESCRIPTION: Disables telemetry service. Use -Undo to set it to Automatic.
# UNDOABLE: Yes
# UNDO_DESC: Sets the telemetry service to Automatic.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Connected User Experiences and Telemetry service set to Automatic." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 4 -Type DWord -Force
        Write-Log "Connected User Experiences and Telemetry service disabled." "INFO"
    }
