...
# DESCRIPTION: Disables desktop composition effects for performance. Use -Undo to enable them.
# UNDOABLE: Yes
# UNDO_DESC: Enables desktop composition effects.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Desktop composition effects are now enabled." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 0 -Type DWord -Force
        Write-Log "Desktop composition effects are now disabled." "INFO"
    }
