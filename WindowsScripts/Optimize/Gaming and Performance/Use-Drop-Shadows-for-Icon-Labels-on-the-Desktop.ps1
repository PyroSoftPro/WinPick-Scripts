...
# DESCRIPTION: Disables desktop icon label shadows. Use -Undo to enable them.
# UNDOABLE: Yes
# UNDO_DESC: Enables desktop icon label shadows.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Desktop icon label shadows enabled." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 0 -Type DWord -Force
        Write-Log "Desktop icon label shadows disabled." "INFO"
    }
