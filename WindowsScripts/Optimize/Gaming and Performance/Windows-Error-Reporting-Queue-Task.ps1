...
# DESCRIPTION: Disables the Windows Error Reporting Queue task. Use -Undo to enable it.
# UNDOABLE: Yes
# UNDO_DESC: Enables the Windows Error Reporting Queue task.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Windows Error Reporting Queue task enabled." "INFO"
    } else {
        Set-TaskState -Name $TaskName -Enable $false
        Write-Log "Windows Error Reporting Queue task disabled." "INFO"
    }
