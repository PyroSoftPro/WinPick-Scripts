...
# DESCRIPTION: Disables media sharing service. Use -Undo to set it to Automatic.
# UNDOABLE: Yes
# UNDO_DESC: Sets media sharing service to Automatic.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Media sharing service set to Automatic." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 4 -Type DWord -Force
        Write-Log "Media sharing service disabled." "INFO"
    }
