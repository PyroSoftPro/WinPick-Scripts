...
# DESCRIPTION: Sets Downloaded Maps Manager to Manual. Use -Undo to set it to Automatic.
# UNDOABLE: Yes
# UNDO_DESC: Sets Downloaded Maps Manager to Automatic.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Downloaded Maps Manager set to Automatic." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 3 -Type DWord -Force
        Write-Log "Downloaded Maps Manager set to Manual." "INFO"
    }
