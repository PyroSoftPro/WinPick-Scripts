...
# DESCRIPTION: Disables network throttling to reduce latency. Use -Undo to restore defaults.
# UNDOABLE: Yes
# UNDO_DESC: Restores default network throttling behavior.
...
try {
    ...
    if ($Undo) {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 5 -Type DWord -Force
        Write-Log "Network throttling restored to defaults." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 10 -Type DWord -Force
        Write-Log "Network throttling disabled for gaming." "INFO"
    }
