...
# DESCRIPTION: Disables cached taskbar thumbnails. Use -Undo to enable them.
# UNDOABLE: Yes
# UNDO_DESC: Enables cached taskbar thumbnails.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Taskbar thumbnail caching enabled." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 0 -Type DWord -Force
        Write-Log "Taskbar thumbnail caching disabled." "INFO"
    }
