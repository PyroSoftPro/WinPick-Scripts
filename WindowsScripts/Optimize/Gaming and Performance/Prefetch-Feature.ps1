...
# DESCRIPTION: Disables Prefetch to reduce background activity. Use -Undo to enable it.
# UNDOABLE: Yes
# UNDO_DESC: Enables Prefetch for boot and application launches.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Prefetch enabled for boot and application launches." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 0 -Type DWord -Force
        Write-Log "Prefetch disabled." "INFO"
    }
