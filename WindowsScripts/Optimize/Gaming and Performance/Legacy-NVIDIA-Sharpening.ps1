...
# DESCRIPTION: Disables legacy NVIDIA sharpening (recommended for modern drivers). Use -Undo to enable it.
# UNDOABLE: Yes
# UNDO_DESC: Enables legacy NVIDIA sharpening.
...
$RegistryPath = "HKLM:\Software\NVIDIA Corporation\Global\FTS"
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Legacy NVIDIA sharpening is now enabled." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 0 -Type DWord -Force
        Write-Log "Legacy NVIDIA sharpening is now disabled." "INFO"
    }
