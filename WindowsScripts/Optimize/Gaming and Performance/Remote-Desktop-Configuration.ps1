...
# DESCRIPTION: Sets Remote Desktop Configuration to Manual. Use -Undo to set it to Automatic.
# UNDOABLE: Yes
# UNDO_DESC: Sets Remote Desktop Configuration to Automatic.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Remote Desktop Configuration set to Automatic." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 3 -Type DWord -Force
        Write-Log "Remote Desktop Configuration set to Manual." "INFO"
    }
