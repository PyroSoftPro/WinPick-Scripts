...
# DESCRIPTION: Disables Remote Access Auto Connection Manager. Use -Undo to set it to Automatic.
# UNDOABLE: Yes
# UNDO_DESC: Sets Remote Access Auto Connection Manager to Automatic.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Remote Access Auto Connection Manager set to Automatic." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 4 -Type DWord -Force
        Write-Log "Remote Access Auto Connection Manager disabled." "INFO"
    }
