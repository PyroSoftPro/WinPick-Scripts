...
# DESCRIPTION: Sets Xbox Live Auth Manager to Manual. Use -Undo to set it to Automatic.
# UNDOABLE: Yes
# UNDO_DESC: Sets Xbox Live Auth Manager to Automatic.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Xbox Live Auth Manager set to Automatic." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 3 -Type DWord -Force
        Write-Log "Xbox Live Auth Manager set to Manual." "INFO"
    }
