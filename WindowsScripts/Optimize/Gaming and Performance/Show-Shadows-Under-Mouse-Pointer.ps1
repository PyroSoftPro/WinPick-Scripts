...
# DESCRIPTION: Disables mouse pointer shadow. Use -Undo to enable it.
# UNDOABLE: Yes
# UNDO_DESC: Enables mouse pointer shadow.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Mouse pointer shadow enabled." "INFO"
    } else {
        Set-UserPreferencesMaskBit -Enable $false
        Write-Log "Mouse pointer shadow disabled." "INFO"
    }
