...
# DESCRIPTION: Disables the FilterKeys hotkey. Use -Undo to enable it.
# UNDOABLE: Yes
# UNDO_DESC: Enables the FilterKeys hotkey.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "FilterKeys hotkey enabled." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value "2" -Type String -Force
        Write-Log "FilterKeys hotkey disabled." "INFO"
    }
