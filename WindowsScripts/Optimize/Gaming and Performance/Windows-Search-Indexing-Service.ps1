...
# DESCRIPTION: Disables Windows Search indexing to reduce background usage. Use -Undo to set it to Automatic.
# UNDOABLE: Yes
# UNDO_DESC: Sets Windows Search indexing to Automatic.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Windows Search indexing set to Automatic." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 4 -Type DWord -Force
        Write-Log "Windows Search indexing disabled." "INFO"
    }
