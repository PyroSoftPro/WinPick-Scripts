...
# DESCRIPTION: Disables the Family Safety Monitor task. Use -Undo to enable it.
# UNDOABLE: Yes
# UNDO_DESC: Enables the Family Safety Monitor task.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Family Safety Monitor task enabled." "INFO"
    } else {
        Set-TaskState -Name $TaskName -Enable $false
        Write-Log "Family Safety Monitor task disabled." "INFO"
    }
