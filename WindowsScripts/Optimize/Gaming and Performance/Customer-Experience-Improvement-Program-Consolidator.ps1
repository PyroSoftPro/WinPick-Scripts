...
# DESCRIPTION: Disables the CEIP Consolidator task. Use -Undo to enable it.
# UNDOABLE: Yes
# UNDO_DESC: Enables the CEIP Consolidator task.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "CEIP Consolidator task enabled." "INFO"
    } else {
        Set-TaskState -Name $TaskName -Enable $false
        Write-Log "CEIP Consolidator task disabled." "INFO"
    }
