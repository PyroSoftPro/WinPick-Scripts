# NAME: USB CEIP Task
# DESCRIPTION: Disables the USB CEIP task. Use -Undo to enable it.
# UNDOABLE: Yes
# UNDO_DESC: Enables the USB CEIP task.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "USB CEIP task enabled." "INFO"
    } else {
        Set-TaskState -Name $TaskName -Enable $false
        Write-Log "USB CEIP task disabled." "INFO"
    }
