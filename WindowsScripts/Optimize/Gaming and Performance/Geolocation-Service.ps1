...
# DESCRIPTION: Disables geolocation service. Use -Undo to set it to Automatic.
# UNDOABLE: Yes
# UNDO_DESC: Sets geolocation service to Automatic.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Geolocation service set to Automatic." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 4 -Type DWord -Force
        Write-Log "Geolocation service disabled." "INFO"
    }
