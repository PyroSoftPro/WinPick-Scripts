...
# DESCRIPTION: Disables the Program Compatibility Assistant Patch task. Use -Undo to enable it.
# UNDOABLE: Yes
# UNDO_DESC: Enables the Program Compatibility Assistant Patch task.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Program Compatibility Assistant Patch task enabled." "INFO"
    } else {
        Set-TaskState -Name $TaskName -Enable $false
        Write-Log "Program Compatibility Assistant Patch task disabled." "INFO"
    }
