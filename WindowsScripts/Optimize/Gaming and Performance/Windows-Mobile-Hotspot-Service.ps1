...
# DESCRIPTION: Sets Mobile Hotspot service to Manual. Use -Undo to set it to Automatic.
# UNDOABLE: Yes
# UNDO_DESC: Sets Mobile Hotspot service to Automatic.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Mobile Hotspot service set to Automatic." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 3 -Type DWord -Force
        Write-Log "Mobile Hotspot service set to Manual." "INFO"
    }
