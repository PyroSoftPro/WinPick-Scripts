...
# DESCRIPTION: Sets Xbox Live Networking Service to Manual. Use -Undo to set it to Automatic.
# UNDOABLE: Yes
# UNDO_DESC: Sets Xbox Live Networking Service to Automatic.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Xbox Live Networking Service set to Automatic." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 3 -Type DWord -Force
        Write-Log "Xbox Live Networking Service set to Manual." "INFO"
    }
