...
# DESCRIPTION: Disables the MAR Backup task. Use -Undo to enable it.
# UNDOABLE: Yes
# UNDO_DESC: Enables the MAR Backup task.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "MAR Backup task enabled." "INFO"
    } else {
        Set-TaskState -Name $TaskName -Enable $false
        Write-Log "MAR Backup task disabled." "INFO"
    }
