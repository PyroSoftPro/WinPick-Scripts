...
# DESCRIPTION: Disables UI element animations for performance. Use -Undo to enable them.
# UNDOABLE: Yes
# UNDO_DESC: Enables UI element animations.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "UI element animations are now enabled." "INFO"
    } else {
        Set-UserPreferencesMaskBit -Enable $false
        Write-Log "UI element animations are now disabled." "INFO"
    }
