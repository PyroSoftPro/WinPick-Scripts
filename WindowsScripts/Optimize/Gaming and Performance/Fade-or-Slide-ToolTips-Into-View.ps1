...
# DESCRIPTION: Disables tooltip animations. Use -Undo to enable them.
# UNDOABLE: Yes
# UNDO_DESC: Enables tooltip animations.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Tooltip animations are now enabled." "INFO"
    } else {
        Set-UserPreferencesMaskBit -Enable $false
        Write-Log "Tooltip animations are now disabled." "INFO"
    }
