...
# DESCRIPTION: Disables the Maps Update task. Use -Undo to enable it.
# UNDOABLE: Yes
# UNDO_DESC: Enables the Maps Update task.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Maps Update task enabled." "INFO"
    } else {
        Set-TaskState -Name $TaskName -Enable $false
        Write-Log "Maps Update task disabled." "INFO"
    }
