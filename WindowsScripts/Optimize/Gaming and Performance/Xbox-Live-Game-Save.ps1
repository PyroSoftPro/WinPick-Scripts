...
# DESCRIPTION: Sets Xbox Live Game Save to Manual. Use -Undo to set it to Automatic.
# UNDOABLE: Yes
# UNDO_DESC: Sets Xbox Live Game Save to Automatic.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Xbox Live Game Save set to Automatic." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 3 -Type DWord -Force
        Write-Log "Xbox Live Game Save set to Manual." "INFO"
    }
