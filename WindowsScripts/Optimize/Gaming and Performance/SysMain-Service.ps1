...
# DESCRIPTION: Disables SysMain to reduce background activity. Use -Undo to set it to Automatic.
# UNDOABLE: Yes
# UNDO_DESC: Sets SysMain to Automatic startup.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "SysMain service set to Automatic." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 4 -Type DWord -Force
        Write-Log "SysMain service disabled." "INFO"
    }
