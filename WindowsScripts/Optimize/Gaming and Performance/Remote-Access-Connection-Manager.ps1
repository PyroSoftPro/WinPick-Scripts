...
# DESCRIPTION: Sets Remote Access Connection Manager to Manual. Use -Undo to set it to Automatic.
# UNDOABLE: Yes
# UNDO_DESC: Sets Remote Access Connection Manager to Automatic.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Remote Access Connection Manager set to Automatic." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 3 -Type DWord -Force
        Write-Log "Remote Access Connection Manager set to Manual." "INFO"
    }
