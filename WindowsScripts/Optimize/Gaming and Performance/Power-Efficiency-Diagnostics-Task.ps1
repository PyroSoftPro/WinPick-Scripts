...
# DESCRIPTION: Disables the Power Efficiency Diagnostics task. Use -Undo to enable it.
# UNDOABLE: Yes
# UNDO_DESC: Enables the Power Efficiency Diagnostics task.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Power Efficiency Diagnostics task enabled." "INFO"
    } else {
        Set-TaskState -Name $TaskName -Enable $false
        Write-Log "Power Efficiency Diagnostics task disabled." "INFO"
    }
