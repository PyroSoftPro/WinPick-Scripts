...
# DESCRIPTION: Raises GPU scheduling priority for games. Use -Undo to restore normal priority.
# UNDOABLE: Yes
# UNDO_DESC: Restores normal GPU priority for games.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "GPU priority for games restored to normal." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 8 -Type DWord -Force
        Write-Log "GPU priority for games set to high." "INFO"
    }
