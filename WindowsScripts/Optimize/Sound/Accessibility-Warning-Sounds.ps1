...
# DESCRIPTION: Disables accessibility warning sounds. Use -Undo to enable them.
# UNDOABLE: Yes
# UNDO_DESC: Enables accessibility warning sounds.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Accessibility warning sounds enabled." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 0 -Type DWord -Force
        Write-Log "Accessibility warning sounds disabled." "INFO"
    }
