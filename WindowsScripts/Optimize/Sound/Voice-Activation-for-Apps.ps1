...
# DESCRIPTION: Disables voice activation for apps. Use -Undo to enable it.
# UNDOABLE: Yes
# UNDO_DESC: Enables voice activation for apps.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Voice activation for apps enabled." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 0 -Type DWord -Force
        Write-Log "Voice activation for apps disabled." "INFO"
    }
