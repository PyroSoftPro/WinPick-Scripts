...
# DESCRIPTION: Disables Windows notifications. Use -Undo to enable them.
# UNDOABLE: Yes
# UNDO_DESC: Enables Windows notifications.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Notifications enabled." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 0 -Type DWord -Force
        Write-Log "Notifications disabled." "INFO"
    }

    Write-Log "You may need to restart the notification service for changes to apply." "INFO"
