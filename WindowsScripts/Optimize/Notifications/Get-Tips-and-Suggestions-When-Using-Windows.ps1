...
# DESCRIPTION: Disables tips and suggestions. Use -Undo to enable them.
# UNDOABLE: Yes
# UNDO_DESC: Enables tips and suggestions.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Tips and suggestions enabled." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 0 -Type DWord -Force
        Write-Log "Tips and suggestions disabled." "INFO"
    }
