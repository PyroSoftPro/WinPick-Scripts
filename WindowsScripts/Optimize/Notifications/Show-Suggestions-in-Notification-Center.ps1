...
# DESCRIPTION: Disables Notification Center suggestions. Use -Undo to enable them.
# UNDOABLE: Yes
# UNDO_DESC: Enables Notification Center suggestions.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Notification Center suggestions enabled." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 0 -Type DWord -Force
        Write-Log "Notification Center suggestions disabled." "INFO"
    }
