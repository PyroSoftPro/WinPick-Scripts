...
# DESCRIPTION: Disables clock change notifications. Use -Undo to enable them.
# UNDOABLE: Yes
# UNDO_DESC: Enables clock change notifications.
...
try {
    ...
    if ($Undo) {
        ...
        Write-Log "Clock change notifications enabled." "INFO"
    } else {
        Set-ItemProperty -Path $RegistryPath -Name $ValueName -Value 0 -Type DWord -Force
        Write-Log "Clock change notifications disabled." "INFO"
    }
