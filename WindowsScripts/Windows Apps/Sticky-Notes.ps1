# NAME: Sticky Notes
# DESCRIPTION: Removes Sticky Notes for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Sticky Notes using WinGet (Microsoft Store).
#
...
function Remove-StickyNotes {
    ...
    Write-Log "Removing installed Sticky Notes packages..." "INFO"
    ...
    Write-Log "Removing provisioned Sticky Notes package..." "INFO"
...
function Install-StickyNotes {
    $wingetId = "9NBLGGH4QGHW"
    $appxName = "Microsoft.MicrosoftStickyNotes"

    $existing = Get-AppxPackage -Name $appxName -AllUsers -ErrorAction SilentlyContinue
    if ($existing) {
        Write-Log "Sticky Notes is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Sticky Notes via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Sticky Notes installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Sticky Notes)..." "INFO"
        Install-StickyNotes
    } else {
