# NAME: Paint
# DESCRIPTION: Removes Paint for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Paint using WinGet (Microsoft Store).
#
...
function Remove-Paint {
    ...
    Write-Log "Removing installed Paint packages..." "INFO"
    ...
    Write-Log "Removing provisioned Paint package..." "INFO"
...
function Install-Paint {
    ...
    if ($existing) {
        Write-Log "Paint is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Paint via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Paint installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Paint)..." "INFO"
        Install-Paint
    } else {
