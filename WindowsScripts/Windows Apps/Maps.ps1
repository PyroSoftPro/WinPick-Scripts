# NAME: Maps
# DESCRIPTION: Removes Microsoft Maps for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Microsoft Maps using WinGet (Microsoft Store).
#
...
function Remove-Maps {
    ...
    Write-Log "Removing installed Maps packages..." "INFO"
    ...
    Write-Log "Removing provisioned Maps package..." "INFO"
...
function Install-Maps {
    $wingetId = "9WZDNCRDTBVB"
    $appxName = "Microsoft.WindowsMaps"

    $existing = Get-AppxPackage -Name $appxName -AllUsers -ErrorAction SilentlyContinue
    if ($existing) {
        Write-Log "Maps is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Maps via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Maps installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Maps)..." "INFO"
        Install-Maps
    } else {
