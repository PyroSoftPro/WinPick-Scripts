# NAME: Bing Search
# DESCRIPTION: Removes Bing Search integration for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Bing Search using WinGet (Microsoft Store).
#
...
function Remove-BingSearch {
    ...
    Write-Log "Removing installed Bing Search packages..." "INFO"
    ...
    Write-Log "Removing provisioned Bing Search package..." "INFO"
...
function Install-BingSearch {
    $wingetId = "9NZBF4GT040C"
    $appxName = "Microsoft.BingSearch"

    $existing = Get-AppxPackage -Name $appxName -AllUsers -ErrorAction SilentlyContinue
    if ($existing) {
        Write-Log "Bing Search is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Bing Search via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Bing Search installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Bing Search)..." "INFO"
        Install-BingSearch
    } else {
