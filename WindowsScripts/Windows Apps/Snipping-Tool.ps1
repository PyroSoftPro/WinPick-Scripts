# NAME: Snipping Tool
# DESCRIPTION: Removes Snipping Tool for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Snipping Tool using WinGet (Microsoft Store).
#
...
function Remove-SnippingTool {
    $appxName = "Microsoft.ScreenSketch"
    ...
    Write-Log "Removing installed Snipping Tool packages..." "INFO"
    ...
    Write-Log "Removing provisioned Snipping Tool package..." "INFO"
...
function Install-SnippingTool {
    $wingetId = "9MZ95KL8MR0L"
    $appxName = "Microsoft.ScreenSketch"

    $existing = Get-AppxPackage -Name $appxName -AllUsers -ErrorAction SilentlyContinue
    if ($existing) {
        Write-Log "Snipping Tool is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Snipping Tool via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Snipping Tool installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Snipping Tool)..." "INFO"
