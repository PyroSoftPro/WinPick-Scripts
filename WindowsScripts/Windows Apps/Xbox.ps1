# NAME: Xbox
# DESCRIPTION: Removes Xbox app for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Xbox app using WinGet (Microsoft Store).
#
...
function Remove-Xbox {
    $appxNames = @("Microsoft.GamingApp", "Microsoft.XboxApp")
    ...
    Write-Log "Removing installed Xbox packages..." "INFO"
    ...
    Write-Log "Removing provisioned Xbox packages..." "INFO"
    foreach ($appxName in $appxNames) {
    ...
    Write-Log "Provisioned packages removed." "INFO"
...
function Install-Xbox {
    ...
    if ($existing) {
        Write-Log "Xbox is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Xbox via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Xbox installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Xbox)..." "INFO"
        Install-Xbox
    } else {
