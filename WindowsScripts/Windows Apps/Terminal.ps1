# NAME: Terminal
# DESCRIPTION: Removes Windows Terminal for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Windows Terminal using WinGet (Microsoft Store).
#
...
function Remove-Terminal {
    ...
    Write-Log "Removing installed Terminal packages..." "INFO"
    ...
    Write-Log "Removing provisioned Terminal package..." "INFO"
...
function Install-Terminal {
    ...
    if ($existing) {
        Write-Log "Terminal is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Terminal via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Terminal installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Terminal)..." "INFO"
        Install-Terminal
    } else {
