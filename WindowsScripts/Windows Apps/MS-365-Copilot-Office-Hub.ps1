...
# DESCRIPTION: Removes MS 365 Copilot (Office Hub) for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls MS 365 Copilot (Office Hub) using WinGet (Microsoft Store).
#
...
function Remove-OfficeHub {
    ...
    Write-Log "Removing installed MS 365 Copilot (Office Hub) packages..." "INFO"
    ...
    Write-Log "Removing provisioned MS 365 Copilot (Office Hub) package..." "INFO"
...
function Install-OfficeHub {
    $wingetId = "9WZDNCRD29V9"
    $appxName = "Microsoft.MicrosoftOfficeHub"

    $existing = Get-AppxPackage -Name $appxName -AllUsers -ErrorAction SilentlyContinue
    if ($existing) {
        Write-Log "MS 365 Copilot (Office Hub) is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing MS 365 Copilot (Office Hub) via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "MS 365 Copilot (Office Hub) installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall MS 365 Copilot (Office Hub))..." "INFO"
        Install-OfficeHub
    } else {
