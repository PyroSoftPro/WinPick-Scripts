# NAME: Clipchamp
# DESCRIPTION: Removes Clipchamp for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Clipchamp using WinGet (Microsoft Store).
#
...
function Remove-Clipchamp {
    ...
    Write-Log "Removing installed Clipchamp packages..." "INFO"
    ...
    Write-Log "Removing provisioned Clipchamp package..." "INFO"
...
function Install-Clipchamp {
    ...
    Write-Log "Installing Clipchamp via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Clipchamp installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Clipchamp)..." "INFO"
        Install-Clipchamp
    } else {
