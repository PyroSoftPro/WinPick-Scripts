# NAME: Movies & TV
# DESCRIPTION: Removes Movies & TV for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Movies & TV using WinGet (Microsoft Store).
#
...
function Remove-MoviesAndTV {
    ...
    Write-Log "Removing installed Movies & TV packages..." "INFO"
    ...
    Write-Log "Removing provisioned Movies & TV package..." "INFO"
...
function Install-MoviesAndTV {
    ...
    Write-Log "Installing Movies & TV via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Movies & TV installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Movies & TV)..." "INFO"
        Install-MoviesAndTV
    } else {
