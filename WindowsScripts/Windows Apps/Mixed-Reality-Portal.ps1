...
# DESCRIPTION: Removes Mixed Reality Portal for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Mixed Reality Portal using WinGet (Microsoft Store).
#
...
function Remove-MixedRealityPortal {
    ...
    Write-Log "Removing installed Mixed Reality Portal packages..." "INFO"
    ...
    Write-Log "Removing provisioned Mixed Reality Portal package..." "INFO"
...
function Install-MixedRealityPortal {
    $wingetId = "9NG1H8B3ZC7M"
    $appxName = "Microsoft.MixedReality.Portal"

    $existing = Get-AppxPackage -Name $appxName -AllUsers -ErrorAction SilentlyContinue
    if ($existing) {
        Write-Log "Mixed Reality Portal is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Mixed Reality Portal via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Mixed Reality Portal installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Mixed Reality Portal)..." "INFO"
        Install-MixedRealityPortal
    } else {
