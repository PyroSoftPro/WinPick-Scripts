# NAME: Power Automate
# DESCRIPTION: Removes Power Automate for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Power Automate using WinGet (Microsoft Store).
#
...
function Remove-PowerAutomate {
    $appxName = "Microsoft.PowerAutomateDesktop"
    ...
    Write-Log "Removing installed Power Automate packages..." "INFO"
    ...
    Write-Log "Removing provisioned Power Automate package..." "INFO"
...
function Install-PowerAutomate {
    $wingetId = "9NFTCH6J7FHV"
    $appxName = "Microsoft.PowerAutomateDesktop"

    $existing = Get-AppxPackage -Name $appxName -AllUsers -ErrorAction SilentlyContinue
    if ($existing) {
        Write-Log "Power Automate is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Power Automate via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Power Automate installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Power Automate)..." "INFO"
