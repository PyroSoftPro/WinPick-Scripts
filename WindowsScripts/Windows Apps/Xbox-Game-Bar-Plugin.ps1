...
# DESCRIPTION: Removes Xbox Game Bar Plugin for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Xbox Game Bar Plugin using WinGet (Microsoft Store).
#
...
function Remove-XboxGameBarPlugin {
    ...
    Write-Log "Removing installed Xbox Game Bar Plugin packages..." "INFO"
    ...
    Write-Log "Removing provisioned Xbox Game Bar Plugin package..." "INFO"
...
function Install-XboxGameBarPlugin {
    $wingetId = "9NBLGGH537C2"
    $appxName = "Microsoft.XboxGameOverlay"

    $existing = Get-AppxPackage -Name $appxName -AllUsers -ErrorAction SilentlyContinue
    if ($existing) {
        Write-Log "Xbox Game Bar Plugin is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Xbox Game Bar Plugin via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Xbox Game Bar Plugin installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Xbox Game Bar Plugin)..." "INFO"
        Install-XboxGameBarPlugin
    } else {
        Write-Log "Starting removal operation..." "INFO"
        Remove-XboxGameBarPlugin
    }
