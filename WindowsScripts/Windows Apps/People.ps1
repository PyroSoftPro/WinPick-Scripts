# NAME: People
# DESCRIPTION: Removes People for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls People using WinGet (Microsoft Store).
#
...
function Remove-People {
    ...
    Write-Log "Removing installed People packages..." "INFO"
    ...
    Write-Log "Removing provisioned People package..." "INFO"
...
function Install-People {
    ...
    Write-Log "Installing People via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "People installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall People)..." "INFO"
        Install-People
    } else {
