...
# DESCRIPTION: Removes Mail and Calendar for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Mail and Calendar using WinGet (Microsoft Store).
#
...
function Remove-MailAndCalendar {
    ...
    Write-Log "Removing installed Mail and Calendar packages..." "INFO"
    ...
    Write-Log "Removing provisioned Mail and Calendar package..." "INFO"
...
function Install-MailAndCalendar {
    $wingetId = "9WZDNCRFHVQM"
    $appxName = "microsoft.windowscommunicationsapps"

    $existing = Get-AppxPackage -Name $appxName -AllUsers -ErrorAction SilentlyContinue
    if ($existing) {
        Write-Log "Mail and Calendar is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Mail and Calendar via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Mail and Calendar installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Mail and Calendar)..." "INFO"
        Install-MailAndCalendar
    } else {
        Write-Log "Starting removal operation..." "INFO"
        Remove-MailAndCalendar
    }
