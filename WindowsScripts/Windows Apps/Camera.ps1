# NAME: Camera
# DESCRIPTION: Removes Windows Camera for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Windows Camera using WinGet (Microsoft Store).
#
...
function Remove-Camera {
    ...
    Write-Log "Removing installed Camera packages..." "INFO"
    ...
    Write-Log "Removing provisioned Camera package..." "INFO"
...
function Install-Camera {
    $wingetId = "9WZDNCRFJBBG"
    $appxName = "Microsoft.WindowsCamera"

    $existing = Get-AppxPackage -Name $appxName -AllUsers -ErrorAction SilentlyContinue
    if ($existing) {
        Write-Log "Camera is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Camera via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Camera installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Camera)..." "INFO"
        Install-Camera
    } else {
