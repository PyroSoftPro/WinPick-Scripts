# NAME: Phone Link
# DESCRIPTION: Removes Phone Link for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Phone Link using WinGet (Microsoft Store).
#
...
function Remove-PhoneLink {
    ...
    Write-Log "Removing installed Phone Link packages..." "INFO"
    ...
    Write-Log "Removing provisioned Phone Link package..." "INFO"
...
function Install-PhoneLink {
    $wingetId = "9NMPJ99VJBWV"
    $appxName = "Microsoft.YourPhone"

    $existing = Get-AppxPackage -Name $appxName -AllUsers -ErrorAction SilentlyContinue
    if ($existing) {
        Write-Log "Phone Link is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Phone Link via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Phone Link installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Phone Link)..." "INFO"
        Install-PhoneLink
    } else {
