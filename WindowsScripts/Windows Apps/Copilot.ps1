# NAME: Copilot
# DESCRIPTION: Removes Copilot for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Copilot using WinGet (Microsoft Store).
#
...
function Remove-Copilot {
    ...
    Write-Log "Removing installed Copilot packages..." "INFO"
    ...
    Write-Log "Removing provisioned Copilot packages..." "INFO"
    foreach ($appxName in $appxNames) {
...
function Install-Copilot {
    ...
    Write-Log "Installing Copilot via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Copilot installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Copilot)..." "INFO"
        Install-Copilot
    } else {
