...
# DESCRIPTION: Removes Xbox Identity Provider for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Xbox Identity Provider using WinGet (Microsoft Store).
#
...
function Remove-XboxIdentityProvider {
    ...
    Write-Log "Removing installed Xbox Identity Provider packages..." "INFO"
    ...
    Write-Log "Removing provisioned Xbox Identity Provider package..." "INFO"
...
function Install-XboxIdentityProvider {
    $wingetId = "9WZDNCRD1HKW"
    $appxName = "Microsoft.XboxIdentityProvider"

    $existing = Get-AppxPackage -Name $appxName -AllUsers -ErrorAction SilentlyContinue
    if ($existing) {
        Write-Log "Xbox Identity Provider is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Xbox Identity Provider via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Xbox Identity Provider installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Xbox Identity Provider)..." "INFO"
        Install-XboxIdentityProvider
    } else {
        Write-Log "Starting removal operation..." "INFO"
        Remove-XboxIdentityProvider
    }
