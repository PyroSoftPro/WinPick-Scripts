# NAME: 3D Viewer
# DESCRIPTION: Removes Microsoft 3D Viewer for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Microsoft 3D Viewer using WinGet (Microsoft Store).
#
...
function Remove-3DViewer {
    $appxName = "Microsoft.Microsoft3DViewer"
    ...
    Write-Log "Removing installed 3D Viewer packages..." "INFO"
    ...
    Write-Log "Removing provisioned 3D Viewer package..." "INFO"
...
function Install-3DViewer {
    $wingetId = "9NBLGGH42THS"
    $appxName = "Microsoft.Microsoft3DViewer"

    $existing = Get-AppxPackage -Name $appxName -AllUsers -ErrorAction SilentlyContinue
    if ($existing) {
        Write-Log "3D Viewer is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing 3D Viewer via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "3D Viewer installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall 3D Viewer)..." "INFO"
        Install-3DViewer
    } else {
