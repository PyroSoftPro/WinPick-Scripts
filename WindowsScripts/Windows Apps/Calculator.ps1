# NAME: Calculator
# DESCRIPTION: Removes Windows Calculator for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Windows Calculator using WinGet (Microsoft Store).
#
...
function Remove-Calculator {
    ...
    Write-Log "Removing installed Calculator packages..." "INFO"
    ...
    Write-Log "Removing provisioned Calculator package..." "INFO"
...
function Install-Calculator {
    ...
    if ($existing) {
        Write-Log "Calculator is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Calculator via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Calculator installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Calculator)..." "INFO"
        Install-Calculator
    } else {
