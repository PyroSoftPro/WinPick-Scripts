...
# DESCRIPTION: Removes Microsoft Solitaire Collection for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Microsoft Solitaire Collection using WinGet (Microsoft Store).
#
...
function Remove-SolitaireCollection {
    ...
    Write-Log "Removing installed Solitaire Collection packages..." "INFO"
    ...
    Write-Log "Removing provisioned Solitaire Collection package..." "INFO"
...
function Install-SolitaireCollection {
    $wingetId = "9WZDNCRFHWD2"
    $appxName = "Microsoft.MicrosoftSolitaireCollection"

    $existing = Get-AppxPackage -Name $appxName -AllUsers -ErrorAction SilentlyContinue
    if ($existing) {
        Write-Log "Solitaire Collection is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Solitaire Collection via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Solitaire Collection installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Solitaire Collection)..." "INFO"
        Install-SolitaireCollection
    } else {
