# NAME: MSN Weather
# DESCRIPTION: Removes MSN Weather for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls MSN Weather using WinGet (Microsoft Store).
#
...
function Remove-MSNWeather {
    ...
    Write-Log "Removing installed MSN Weather packages..." "INFO"
    ...
    Write-Log "Removing provisioned MSN Weather package..." "INFO"
...
function Install-MSNWeather {
    $wingetId = "9WZDNCRFJ3Q2"
    $appxName = "Microsoft.BingWeather"

    $existing = Get-AppxPackage -Name $appxName -AllUsers -ErrorAction SilentlyContinue
    if ($existing) {
        Write-Log "MSN Weather is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing MSN Weather via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "MSN Weather installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall MSN Weather)..." "INFO"
        Install-MSNWeather
    } else {
