# NAME: Sound Recorder
# DESCRIPTION: Removes Sound Recorder for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Sound Recorder using WinGet (Microsoft Store).
#
...
function Remove-SoundRecorder {
    ...
    Write-Log "Removing installed Sound Recorder packages..." "INFO"
    ...
    Write-Log "Removing provisioned Sound Recorder package..." "INFO"
...
function Install-SoundRecorder {
    $wingetId = "9WZDNCRFHWKN"
    $appxName = "Microsoft.WindowsSoundRecorder"

    $existing = Get-AppxPackage -Name $appxName -AllUsers -ErrorAction SilentlyContinue
    if ($existing) {
        Write-Log "Sound Recorder is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Sound Recorder via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Sound Recorder installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Sound Recorder)..." "INFO"
