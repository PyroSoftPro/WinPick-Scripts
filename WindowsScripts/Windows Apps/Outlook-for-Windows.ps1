...
# DESCRIPTION: Removes Outlook for Windows for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Outlook for Windows using WinGet (Microsoft Store).
#
...
function Remove-OutlookForWindows {
    ...
    Write-Log "Removing installed Outlook for Windows packages..." "INFO"
    ...
    Write-Log "Removing provisioned Outlook for Windows package..." "INFO"
...
function Install-OutlookForWindows {
    $wingetId = "9NRX63209R7B"
    $appxName = "Microsoft.OutlookForWindows"

    $existing = Get-AppxPackage -Name $appxName -AllUsers -ErrorAction SilentlyContinue
    if ($existing) {
        Write-Log "Outlook for Windows is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Outlook for Windows via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Outlook for Windows installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Outlook for Windows)..." "INFO"
        Install-OutlookForWindows
    } else {
