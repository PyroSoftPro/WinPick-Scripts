...
# DESCRIPTION: Removes Microsoft Family Safety for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Microsoft Family Safety using WinGet (Microsoft Store).
#
...
function Remove-MicrosoftFamilySafety {
    $appxName = "MicrosoftCorporationII.MicrosoftFamily"
    ...
    Write-Log "Removing installed Microsoft Family Safety packages..." "INFO"
    ...
    Write-Log "Removing provisioned Microsoft Family Safety package..." "INFO"
...
function Install-MicrosoftFamilySafety {
    $wingetId = "9PDJDJS743XF"
    $appxName = "MicrosoftCorporationII.MicrosoftFamily"

    $existing = Get-AppxPackage -Name $appxName -AllUsers -ErrorAction SilentlyContinue
    if ($existing) {
        Write-Log "Microsoft Family Safety is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Microsoft Family Safety via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Microsoft Family Safety installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Microsoft Family Safety)..." "INFO"
        Install-MicrosoftFamilySafety
    } else {
        Write-Log "Starting removal operation..." "INFO"
        Remove-MicrosoftFamilySafety
    }
