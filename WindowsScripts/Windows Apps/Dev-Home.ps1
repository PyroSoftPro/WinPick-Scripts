# NAME: Dev Home
# DESCRIPTION: Removes Dev Home for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Dev Home using WinGet (Microsoft Store).
#
...
function Remove-DevHome {
    $appxName = "Microsoft.Windows.DevHome"
    ...
    Write-Log "Removing installed Dev Home packages..." "INFO"
    ...
    Write-Log "Removing provisioned Dev Home package..." "INFO"
...
function Install-DevHome {
    $wingetId = "9N8MHTPHNGVV"
    $appxName = "Microsoft.Windows.DevHome"

    $existing = Get-AppxPackage -Name $appxName -AllUsers -ErrorAction SilentlyContinue
    if ($existing) {
        Write-Log "Dev Home is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Dev Home via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Dev Home installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Dev Home)..." "INFO"
        Install-DevHome
    } else {
