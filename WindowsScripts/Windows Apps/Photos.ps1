# NAME: Photos
# DESCRIPTION: Removes Photos for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Photos using WinGet (Microsoft Store).
#
...
function Remove-Photos {
    ...
    Write-Log "Removing installed Photos packages..." "INFO"
    ...
    Write-Log "Removing provisioned Photos package..." "INFO"
...
function Install-Photos {
    $wingetId = "9WZDNCRFJBH4"
    $appxName = "Microsoft.Windows.Photos"

    $existing = Get-AppxPackage -Name $appxName -AllUsers -ErrorAction SilentlyContinue
    if ($existing) {
        Write-Log "Photos is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Photos via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Photos installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Photos)..." "INFO"
        Install-Photos
    } else {
