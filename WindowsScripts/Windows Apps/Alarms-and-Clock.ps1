# NAME: Alarms & Clock
# DESCRIPTION: Removes Alarms & Clock for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Alarms & Clock using WinGet (Microsoft Store).
#
...
function Remove-AlarmsClock {
    ...
    Write-Log "Removing installed Alarms & Clock packages..." "INFO"
    ...
    Write-Log "Removing provisioned Alarms & Clock package..." "INFO"
...
function Install-AlarmsClock {
    $wingetId = "9WZDNCRFJ3PR"
    $appxName = "Microsoft.WindowsAlarms"

    $existing = Get-AppxPackage -Name $appxName -AllUsers -ErrorAction SilentlyContinue
    if ($existing) {
        Write-Log "Alarms & Clock is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Alarms & Clock via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Alarms & Clock installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Alarms & Clock)..." "INFO"
        Install-AlarmsClock
    } else {
