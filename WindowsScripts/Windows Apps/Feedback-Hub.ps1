# NAME: Feedback Hub
# DESCRIPTION: Removes Feedback Hub for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Feedback Hub using WinGet (Microsoft Store).
#
...
function Remove-FeedbackHub {
    ...
    Write-Log "Removing installed Feedback Hub packages..." "INFO"
    ...
    Write-Log "Removing provisioned Feedback Hub package..." "INFO"
...
function Install-FeedbackHub {
    $wingetId = "9NBLGGH4R32N"
    $appxName = "Microsoft.WindowsFeedbackHub"

    $existing = Get-AppxPackage -Name $appxName -AllUsers -ErrorAction SilentlyContinue
    if ($existing) {
        Write-Log "Feedback Hub is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Feedback Hub via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Feedback Hub installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Feedback Hub)..." "INFO"
        Install-FeedbackHub
    } else {
