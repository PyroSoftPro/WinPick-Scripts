...
# DESCRIPTION: Removes Microsoft Store for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Microsoft Store using WinGet (Microsoft Store).
#
...
function Remove-MicrosoftStore {
    ...
    Write-Log "Removing installed Microsoft Store packages..." "INFO"
    ...
    Write-Log "Removing provisioned Microsoft Store package..." "INFO"
...
function Install-MicrosoftStore {
    ...
    if ($existing) {
        Write-Log "Microsoft Store is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Microsoft Store via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Microsoft Store installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Microsoft Store)..." "INFO"
