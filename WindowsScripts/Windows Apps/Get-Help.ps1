# NAME: Get Help
# DESCRIPTION: Removes Get Help for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Get Help using WinGet (Microsoft Store).
#
...
function Remove-GetHelp {
    ...
    Write-Log "Removing installed Get Help packages..." "INFO"
    ...
    Write-Log "Removing provisioned Get Help package..." "INFO"
...
function Install-GetHelp {
    ...
    Write-Log "Installing Get Help via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Get Help installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Get Help)..." "INFO"
        Install-GetHelp
    } else {
