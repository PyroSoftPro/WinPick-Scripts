# NAME: OneNote
# DESCRIPTION: Removes OneNote for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls OneNote using WinGet (Microsoft Store).
#
...
function Remove-OneNote {
    $appxName = "Microsoft.Office.OneNote"
    ...
    Write-Log "Removing installed OneNote packages..." "INFO"
    ...
    Write-Log "Removing provisioned OneNote package..." "INFO"
...
function Install-OneNote {
    $wingetId = "XPFFZHVGQWWLHB"
    $appxName = "Microsoft.Office.OneNote"

    $existing = Get-AppxPackage -Name $appxName -AllUsers -ErrorAction SilentlyContinue
    if ($existing) {
        Write-Log "OneNote is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing OneNote via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "OneNote installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall OneNote)..." "INFO"
        Install-OneNote
    } else {
