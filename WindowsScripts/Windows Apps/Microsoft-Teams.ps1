...
# DESCRIPTION: Removes Microsoft Teams for all users and deprovisions it. Use -Undo to reinstall via WinGet.
# UNDOABLE: Yes
# UNDO_DESC: Reinstalls Microsoft Teams using WinGet.
#
...
function Remove-MicrosoftTeams {
    ...
    Write-Log "Removing installed Microsoft Teams packages..." "INFO"
    ...
    Write-Log "Removing provisioned Microsoft Teams package..." "INFO"
...
function Install-MicrosoftTeams {
    $wingetId = "XP8BT8DW290MPQ"
    $appxName = "MSTeams"

    $existing = Get-AppxPackage -Name $appxName -AllUsers -ErrorAction SilentlyContinue
    if ($existing) {
        Write-Log "Microsoft Teams is already installed. No action taken." "INFO"
        return
    }
    ...
    Write-Log "Installing Microsoft Teams via WinGet..." "INFO"
    $args = @(
        "install",
        "--id", $wingetId,
        "--source", "msstore",
    ...
    Write-Log "Microsoft Teams installed successfully." "INFO"
}

try {
    Ensure-Admin
    if ($Undo) {
        Write-Log "Starting undo operation (reinstall Microsoft Teams)..." "INFO"
